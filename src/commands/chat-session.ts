...
import {
  ...
  enforceStorageLimit,
  formatBytes,
  getStorageGuidanceLines,
} from '../utils/storage-limit.js';
...
export async function startChatSession(options: AgentOptions): Promise<void> {
  ...
  if (!isQuiet) {
    ...
    if (storageInfo.usagePercent > 80 && !storageInfo.needsCleanup) {
      ...
      console.log(chalk.gray('   Run /storage for platform-specific guidance and cleanup options\n'));
    }
  }
    ...
    if (userInput.toLowerCase() === '/storage') {
      try {
        ...
        if (info.needsCleanup) {
          ...
          getStorageGuidanceLines(configDir).forEach((line) => {
            console.log(chalk.gray(`  • ${line}`));
          });
          console.log(chalk.gray('  • Auto-cleanup runs when the limit is exceeded\n'));
        } else {
