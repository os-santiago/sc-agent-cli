...
type SessionPermissionMode = 'ask_once' | 'always_ask' | 'unlimited';

export function getInfoSummaryRows(
  ...
  const rows = [
    { label: 'Profile', value: currentConfig.activeProfile || 'none' },
    ...
    { label: 'Permissions', value: permissionMode },
...
export async function startChatSession(options: AgentOptions): Promise<void> {
  ...
  let currentPermissionMode: SessionPermissionMode = options.autoApprove ? 'unlimited' : 'ask_once';
  ...
  while (true) {
    ...
    if (userInput.toLowerCase() === '/info') {
      ...
      for (const row of getInfoSummaryRows(currentConfig, currentPermissionMode, history.length)) {
        console.log(chalk.white(`  ${row.label.padEnd(18)} `) + chalk.gray(row.value));
      }
      console.log();
      continue;
    }
