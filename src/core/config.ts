...
export async function initConfig(force = false): Promise<void> {
  if (!force) {
    try {
      await readFile(getGlobalConfigPath(), 'utf-8');
      throw new Error(
        `Global config already exists at ${getGlobalConfigPath()}. ` +
...
export async function setActiveProfile(
  ...
  const globalConfig = (await readConfigFile(globalConfigPath, 'global')) ?? {};

  if (projectRoot) {
    const projectConfigPath = path.join(projectRoot, '.sc-agent.json');
    const projectConfig = await readConfigFile(projectConfigPath, 'project');

    if (projectConfig?.profiles && Object.hasOwn(projectConfig.profiles, profileName)) {
      await saveConfig({ ...projectConfig, activeProfile: profileName }, false, projectRoot);
      return 'project';
    }

    if (projectConfig && Object.hasOwn(projectConfig, 'activeProfile')) {
      await saveConfig({ ...projectConfig, activeProfile: undefined }, false, projectRoot);
    }
  }

  await saveConfig({ ...globalConfig, activeProfile: profileName }, true);
  return 'global';
}
...
async function mergeConfigFile(
  ...
  const parsedConfig = await readConfigFile(configPath, scope);
  if (!parsedConfig) {
    return config;
  }
...
async function readConfigFile(
  configPath: string,
  scope: ConfigScope
): Promise<Partial<ProjectConfig> | undefined> {
  let data: string;
