...
If the config file already exists, `sc config-init` stops without overwriting it.
Use `sc config-init --force` only when you intentionally want to replace the global config.
